...
export function donationConfirmationEmail(input: DonationConfirmationInput) {
  ...
  const text = [
    ...
    `${formattedAmount} gift. Click the link below within 24 hours to`,
...
<body style="font-family: -apple-system, system-ui, sans-serif; color: #0d0407; line-height: 1.55; max-width: 560px; margin: 0 auto; padding: 24px;">
  <p>${escape(greeting)}</p>
  <p>Thanks for choosing to support Third Eye Worldwide with a <strong>${escape(cadence)}</strong> <strong>${escape(formattedAmount)}</strong> gift. Click the link below within 24 hours to confirm your interest — once confirmed, a team member will be in touch shortly to complete your gift through a secure checkout.</p>
