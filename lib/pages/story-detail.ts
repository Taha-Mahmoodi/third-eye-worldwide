...
export function renderStoryDetail(content: SiteContent | null | undefined, slug?: string): string {
  ...
  const tagLine = _esc(story.eyebrow || 'Featured Story · 12 min');
  const title = _rich(story.title || 'Story');
  const standfirst = _esc(story.desc || story.excerpt || '');
  const author = _esc(story.author || 'Maya Osei');
  const initials = _esc(story.initials || author.split(' ').map((w) => w[0] || '').slice(0, 2).join('') || 'TE');
  const role = _esc(story.role || 'TEWW Storyteller');
  const dateStr = _esc(story.date || 'April 17, 2026');
  const location = _esc(story.location || 'Lagos, Nigeria');
  const readTime = _esc(story.meta || story.readTime || '12 min read');
  ...
  <header class="story-hero">
    ...
    <div class="story-hero-inner">
      <div class="story-hero-crumbs">
        ...
        <span class="sep">/</span>
        <span class="curr">${location}</span>
      </div>
      <span class="story-hero-tag"><span class="dot"></span> ${tagLine}</span>
      <h1 class="story-hero-title">${title}</h1>
      ${standfirst ? `<p class="story-hero-stand">${standfirst}</p>` : ''}
...
// Reader progress bar setup lives in lib/client-init.js.
